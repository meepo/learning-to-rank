...
for fold = 1 : 5
    ...
    for loop = 1 : T
        ...
        for i = 1 : length(Y)
            [~, index] = sort(Y{i}, 'descend');     % 对一类查询的样本按照Y从大到小排序
            tmpX = zeros(length(Y{i}), length(w));  

            for j = 1 : length(Y{i})
                tmpX(j, :) = X(cnt + index(j), :);  % 记录每一类查询中排好序的每个样本的Y对应的X
            end

            product = tmpX * w;                     % 叉乘
            totalexp = zeros(1, 3);                 % 记录总的exp得到的和
            tmpexp = exp(product);
            totalexp(1) = sum(tmpexp);              %1,2,3分别对应top3模型中的三个exp的和
            totalexp(2) = totalexp(1) - exp(product(1));
            totalexp(3) = totalexp(2) - exp(product(2));

            result = zeros(1, 3);
            tmpresult = tmpexp' * tmpX;   % 按照公式计算exp与X的乘积
